...
def run(
    ...
    # ---------- Plot scatter (Quality vs Price) ----------
    # Prepare plotting values with optional right-tail clip for readability
    x = df["rel_price"].values.copy()
    y = df["rel_quality"].values.copy()
    if percentile_clip_x is not None and np.isfinite(x).any():
        cap = np.nanpercentile(x, float(percentile_clip_x))
        x = np.clip(x, 1e-9, cap)  # keep >0 for log scale

    plt.figure(figsize=(11, 7))
    plt.scatter(x, y, s=18, alpha=0.6, edgecolor="none")
    ...
    if log_x:
        plt.xscale("log")
        # annotate a few reference x-ticks
        try:
            plt.xticks([0.5, 1.0, 1.5, 2.0], ["0.5×", "1×", "1.5×", "2×"])
        except Exception:
            pass
    ...
    if by_sector:
        _plot_sector_subplot(
            ...
            out_dir=out_dir,
            log_x=log_x,
            percentile_clip_x=percentile_clip_x,
            label_top_n=6,
        )
